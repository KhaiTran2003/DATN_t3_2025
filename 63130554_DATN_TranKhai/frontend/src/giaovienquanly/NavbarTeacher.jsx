...
const NavbarTeacher = () => {
  ...
  const handleDangXuat = () => {
    ...
    localStorage.removeItem('userGV'); // nếu có
    setUser(null);
    setShowDropdown(false);
    navigate('/'); // hoặc window.location.href = '/'
  };
  
  
  // Tải thông tin giáo viên từ API sau khi lấy maGV từ localStorage
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        console.log("Thông tin giáo viên:", res.data);
        setUser(res.data);
      } catch (err) {
      ...
      <div className="navbar-right-teacher" ref={dropdownRef}>
        ...
        {showDropdown && (
          <div className="dropdown-menu-teacher">
            ...
            <button onClick={() => window.location.href = '/teacher/hosocanhan'}>Hồ sơ</button>
            <button onClick={() => {
